...
<body id="page-top">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <!-- Main Content -->
        ...
        <div class="container-fluid">      
          ...
          <!-- Content Row -->
          <h1 class="h3 mb-2 text-gray-800">Ajouter de nouveau document de GEEA</h1><br>
  ...
  <?php 
        ...
        $req = $db->query('SELECT licence1, licence2, licence3 FROM nom_matiere_geea');
  ?>
        
      <div class="input-file-contrainer">
        <form method="post" action="share-geea.php" enctype="multipart/form-data">
    ...
    foreach ($_FILES['fichier']['name'] as $key => $value){
      ...
      $file_name = basename($_FILES['fichier']['name'][$key]);
        ...
        if(in_array($extension_upload,$extension_autoriser)){
          $reponse = $db2->query('SELECT path_doc FROM document_geea');
            $existance = false;
            while($donnees = $reponse->fetch()){
              if('admin/upload/geea/'.$file_name == $donnees['path_doc']){
                  $existance = true;
              }
            }
            if($existance){
            ...
            <?php
              ...
              move_uploaded_file($_FILES['fichier']['tmp_name'][$key],'../../../upload/geea/'.$file_name);
              $insertion = $db2->prepare('INSERT INTO document_geea(name,path_doc,size,type,nom_matiere,var_matiere,filiere) VALUES(:name,:path_doc,:size,:type,:nom_matiere,:var_matiere,:filiere)');
              $insertion->execute(array('name'=>$file_name,'path_doc'=>'admin/upload/geea/'.$file_name,'size'=>($file_size/1000000.0),'type'=>'admin','nom_matiere'=>$nom_matiere,'var_matiere'=>$var_matiere,'filiere'=>'geea'));
            ?>
            <script type="text/javascript">
              var file_name='<?= $file_name ?>';
