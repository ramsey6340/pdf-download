...
<body>
	...
	<?php 
		...
		if(isset($_GET['matiere'])){
			...
			if($variable_trouver == false){
				$reponse = $bdd->query('SELECT ID,licence1 FROM variable_matiere_git');
				$champ='l1';
			...
			if($variable_trouver == false){
				$reponse = $bdd->query('SELECT ID,licence2 FROM variable_matiere_git');
				$champ='l2';
				while ($donnees = $reponse->fetch()) {
			...
			if($variable_trouver == false){
				$reponse = $bdd->query('SELECT ID,licence3 FROM variable_matiere_git');
				$champ='l3';
				while ($donnees = $reponse->fetch()) {
			...
			if($variable_trouver == false){
				$reponse = $bdd->query('SELECT ID,master1 FROM variable_matiere_git');
				$champ='m1';
				while ($donnees = $reponse->fetch()) {
					if($donnees['master1']==$_GET['matiere']){
						$id_variable = $donnees['ID'];
						$variable_trouver = true; 
					}
				}
			...
			if($variable_trouver == false){
				$reponse = $bdd->query('SELECT ID,master2 FROM variable_matiere_git');
				$champ='m2';
				while ($donnees = $reponse->fetch()) {
					if($donnees['master2']==$_GET['matiere']){
						$id_variable = $donnees['ID'];
						$variable_trouver = true; 
					}
				}
			...
			if($variable_trouver == true){
				switch($champ){
					case 'l1': 
						$reponse = $bdd->prepare('SELECT ID,licence1 FROM nom_matiere_git WHERE ID=:id_var');
						$reponse->execute(array('id_var'=>$id_variable));
						$donnees = $reponse->fetch();
						$matiere = $donnees['licence1'];
						break;
					case 'l2':
						$reponse = $bdd->prepare('SELECT ID,licence2 FROM nom_matiere_git WHERE ID=:id_var');
						$reponse->execute(array('id_var'=>$id_variable));
						$donnees = $reponse->fetch();
						$matiere = $donnees['licence2'];
						break;
					case 'l3':
						$reponse = $bdd->prepare('SELECT ID,licence3 FROM nom_matiere_git WHERE ID=:id_var');
						$reponse->execute(array('id_var'=>$id_variable));
						$donnees = $reponse->fetch();
						$matiere = $donnees['licence3'];
						break;
					case 'm1':
						$reponse = $bdd->prepare('SELECT ID,master1 FROM nom_matiere_git WHERE ID=:id_var');
						$reponse->execute(array('id_var'=>$id_variable));
						$donnees = $reponse->fetch();
						$matiere = $donnees['master1'];
						break;
					case 'm2':
						$reponse = $bdd->prepare('SELECT ID,master2 FROM nom_matiere_git WHERE ID=:id_var');
						$reponse->execute(array('id_var'=>$id_variable));
						$donnees = $reponse->fetch();
						$matiere = $donnees['master2'];
						break;
					default:
						echo 'Error';
				}
	...
	<div class="telecharge">
		...
		if(isset($_GET['matiere'])){
			$reponse = $bdd->prepare('SELECT path_doc,name,ROUND(size,2) AS round_size FROM document_git WHERE nom_matiere=:nom_matiere');
