...
<body>
	...
	<?php 
		...
		if(isset($_GET['matiere'])){
			...
			if($variable_trouver == false){
				$reponse = $bdd->query('SELECT ID,licence1 FROM variable_matiere_topo');
				$champ='l1';
			...
			if($variable_trouver == false){
				$reponse = $bdd->query('SELECT ID,licence2 FROM variable_matiere_topo');
				$champ='l2';
				while ($donnees = $reponse->fetch()) {
			...
			if($variable_trouver == false){
				$reponse = $bdd->query('SELECT ID,licence3 FROM variable_matiere_topo');
				$champ='l3';
			...
			if($variable_trouver == true){
				switch($champ){
					case 'l1': 
						$reponse = $bdd->prepare('SELECT ID,licence1 FROM nom_matiere_topo WHERE ID=:id_var');
						$reponse->execute(array('id_var'=>$id_variable));
						$donnees = $reponse->fetch();
						$matiere = $donnees['licence1'];
						break;
					case 'l2':
						$reponse = $bdd->prepare('SELECT ID,licence2 FROM nom_matiere_topo WHERE ID=:id_var');
						$reponse->execute(array('id_var'=>$id_variable));
						$donnees = $reponse->fetch();
						$matiere = $donnees['licence2'];
						break;
					case 'l3':
						$reponse = $bdd->prepare('SELECT ID,licence3 FROM nom_matiere_topo WHERE ID=:id_var');
						$reponse->execute(array('id_var'=>$id_variable));
						$donnees = $reponse->fetch();
	...
	<div class="telecharge">
		...
		if(isset($_GET['matiere'])){
			$reponse = $bdd->prepare('SELECT path_doc,name,ROUND(size,2) AS round_size FROM document_topo WHERE nom_matiere=:nom_matiere');
