...
<body>
	...
	<h2>Partager des documents de Geologie</h2>
      ...
      if(($size = (double)$_FILES['fichier']['size'])<=5000000){
        ...
        if(in_array($extension_upload,$extension_autoriser)){
          ...
          while ($donnees = $reponse->fetch()){
            if('admin/upload/geo/'.$path_doc == $donnees['path_doc']){
              $existance = true;
            }
          }
          ...
          if($existance){
            ...
            $insertion->execute(array('name'=>$name,'path_doc'=>'admin/upload/tmp/'.$name,'size'=>($size/1000000.0),'type'=>'admin','nom_matiere'=>$nom_matiere,'var_matiere'=>'Inconnue','filiere'=>'geo'));
            ?>
            <script type="text/javascript">
