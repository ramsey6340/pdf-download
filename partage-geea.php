...
<body>
	...
	<h2>Partager des documents de Genie Electrique Electrinique et Automatisme (GEEA)</h2>
	...
	<?php 
		if(isset($_SESSION['email'])){
			...
			<div class="input-file-contrainer">
				<form method="post" action="partage-geea.php" enctype="multipart/form-data">
      ...
      if(($size = (double)$_FILES['fichier']['size'])<=5000000){
        ...
        if(in_array($extension_upload,$extension_autoriser)){
          ...
          while ($donnees = $reponse->fetch()){
            if('admin/upload/geea/'.$path_doc == $donnees['path_doc']){
              $existance = true;
            }
          }
          ...
          if($existance){
            ...
            $insertion->execute(array('name'=>$name,'path_doc'=>'admin/upload/tmp/'.$name,'size'=>($size/1000000.0),'type'=>'admin','nom_matiere'=>$nom_matiere,'var_matiere'=>'Inconnue','filiere'=>'geea'));
            ?>
            <script type="text/javascript">
