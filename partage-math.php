...
<body>
	...
	<h2>Partager des documents de Mathématique</h2>
	...
	<?php 
		if(isset($_SESSION['email'])){
			...
			<div class="input-file-contrainer">
				<form method="post" action="partage-math.php" enctype="multipart/form-data">
					<select name="classe">
    ...
    if(isset($_FILES['fichier']) AND $_POST['classe'] AND $_FILES['fichier']['error']==0){
      ...
      if(($size = (double)$_FILES['fichier']['size'])<=5000000){
        ...
        if(in_array($extension_upload,$extension_autoriser)){
          $reponse = $db->query('SELECT path_doc FROM document_math');
          $existance = false;
          while ($donnees = $reponse->fetch()){
            if('admin/upload/math/'.$path_doc == $donnees['path_doc']){
              $existance = true;
            }
          }
          ...
          if($existance){
            ...
            $insertion->execute(array('name'=>$name,'path_doc'=>'admin/upload/tmp/'.$name,'size'=>($size/1000000.0),'type'=>'admin','classe'=>$classe,'filiere'=>'SF-M'));
            ?>
            <script type="text/javascript">
