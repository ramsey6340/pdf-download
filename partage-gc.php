...
<body>
	...
	<h2>Partager des documents de Genie Civil (GC)</h2>
	...
	<?php 
		if(isset($_SESSION['email'])){
			...
			<div class="input-file-contrainer">
				<form method="post" action="partage-gc.php" enctype="multipart/form-data">
      ...
      if(($size = (double)$_FILES['fichier']['size'])<=5000000){
        ...
        if(in_array($extension_upload,$extension_autoriser)){
          ...
          while ($donnees = $reponse->fetch()){
            if('admin/upload/gc/'.$path_doc == $donnees['path_doc']){
              $existance = true;
            }
          }
          ...
          if($existance){
            ...
            $insertion->execute(array('name'=>$name,'path_doc'=>'admin/upload/tmp/'.$name,'size'=>($size/1000000.0),'type'=>'admin','nom_matiere'=>$nom_matiere,'var_matiere'=>'Inconnue','filiere'=>'gc'));
            ?>
            <script type="text/javascript">
