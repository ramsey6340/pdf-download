...
<body>
	...
	<h2>Partager des documents de Chimie</h2>
	...
	<?php 
		if(isset($_SESSION['email'])){
				...
				<form method="post" action="partage-chi.php" enctype="multipart/form-data">
					<select name="classe">
...
<!-- ==================Traitements des fichier partagé================= -->
    ...
    if(isset($_FILES['fichier']) AND $_POST['classe'] AND $_FILES['fichier']['error']==0){
    	$classe = $_POST['classe'];
      if(($size = (double)$_FILES['fichier']['size'])<=5000000){
        ...
        if(in_array($extension_upload,$extension_autoriser)){
          $reponse = $db->query('SELECT path_doc FROM document_chi');
          $existance = false;
          while ($donnees = $reponse->fetch()){
            if('admin/upload/chimie/'.$path_doc == $donnees['path_doc']){
              $existance = true;
            }
          }
          ...
          if($existance){
            ...
            move_uploaded_file($_FILES['fichier']['tmp_name'],'admin/upload/chimie/'.$name);
            ...
            $insertion->execute(array('name'=>$name,'path_doc'=>'admin/upload/chimie/'.$name,'size'=>($size/1000000.0),'type'=>'admin','classe'=>$classe,'filiere'=>'SF-C'));
            ?>
            <script type="text/javascript">
